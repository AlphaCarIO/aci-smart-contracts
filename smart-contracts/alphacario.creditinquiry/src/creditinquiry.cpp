...
ACTION creditinquiry::notify(uint64_t id, const std::string & hash_val) {
  require_auth(get_self());
  require_recipient(get_self());
}

ACTION creditinquiry::insert(const std::string & hash_val) {
  require_auth(get_self());
...
void creditinquiry::send_summary(uint64_t id, const std::string & hash_val) {
  action(
    permission_level{get_self(), "active"_n},
    get_self(),
    "notify"_n,
    std::make_tuple(id, hash_val)
  ).send();
};
